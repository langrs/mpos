...
import com.site.core.base.BaseModel;
...
public abstract class MyBatisDaoImpl<PK extends Serializable, T extends Serializable> extends AbstractDaoImpl<PK, T> {
	...
	public void create(T entity) {
		if (entity instanceof AbstractModel) {
			...
			if (model.getId() == null || "0".equals(model.getId())) {
				model.setId(idGenerator.getSuid());
			}
			if (model.getCreateBy()==null||model.getCreateBy().equals("")) {
				com.site.entity.User dbUser=userService.getByUserNo();
				model.setCreateBy(dbUser.getId()+"");
			}
...
//			User dbUser=userService.getByUserNo();
//			model.setUpdateBy(dbUser.getId()+"");
