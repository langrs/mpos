...
<body>
	...
	<ol class="breadcrumb">
		...
		<button type="button" class="btn btn-warning btn-sm bing-btn-sm nav-a"
			onclick="delete_check('${contextPath}/tRole/delete.do','listTable');">删除</button>
		<a type="button" class="btn btn-success btn-sm bing-btn-sm nav-a"
			href="${contextPath}/tRole/add.do" target="mainIframe">新增</a>
	</ol>

	<form:form id="listForm" name="listForm" modelAttribute="tRole"
		action="${contextPath}/tRole/list.do" method="get" class="form-inline">
		<div class="row">
				<form:input class="query-input" path="id" placeholder="ID" />
				<form:input class="query-input" path="roleType" placeholder="角色类型" />
				<form:input class="query-input" path="roleNo" placeholder="角色编码" />
				<form:input class="query-input" path="name" placeholder="角色名称" />
						...
						<tr onclick="setBox(this);">
							<td><input name="singleCkb" type="checkbox"
									data-ckb-id="${entity.id}"></td>
									 ...
									 <td>${entity.isFlag}</td>
                                                       <td><a href="${contextPath }/tRole/view.do?id=${entity.id}"
								style="margin-left: 0px;">查看</a><a
								href="${contextPath }/tRole/edit.do?id=${entity.id}">修改</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
