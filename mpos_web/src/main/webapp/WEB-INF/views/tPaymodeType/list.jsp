...
<body>
	...
	<ol class="breadcrumb">
		...
		<button type="button" class="btn btn-warning btn-sm bing-btn-sm nav-a"
			onclick="delete_check('${contextPath}/tPaymodeType/delete.do','listTable');">删除</button>
		<a type="button" class="btn btn-success btn-sm bing-btn-sm nav-a"
			href="${contextPath}/tPaymodeType/add.do" target="mainIframe">新增</a>
	</ol>

	<form:form id="listForm" name="listForm" modelAttribute="tPaymodeType"
		action="${contextPath}/tPaymodeType/list.do" method="get" class="form-inline">
		<div class="row">
				<form:input class="query-input" path="id" placeholder="ID" />
				<form:input class="query-input" path="typeNo" placeholder="类别编码" />
				<form:input class="query-input" path="name" placeholder="类别名称" />
				<form:input class="query-input" path="typeAttr" placeholder="类别属性" />
						...
						<tr onclick="setBox(this);">
							<td><input name="singleCkb" type="checkbox"
									data-ckb-id="${entity.id}"></td>
									 ...
									 <td>${entity.isFlag}</td>
                                                       <td><a href="${contextPath }/tPaymodeType/view.do?id=${entity.id}"
								style="margin-left: 0px;">查看</a><a
								href="${contextPath }/tPaymodeType/edit.do?id=${entity.id}">修改</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
